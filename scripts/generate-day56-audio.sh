#!/usr/bin/env bash
# generate-day56-audio.sh
#
# Full pipeline: Day 56 diary audio — FR + EN
...
#
# Usage:
#   FAL_KEY=<key> bash scripts/generate-day56-audio.sh
#   or: source .env.local && bash scripts/generate-day56-audio.sh
...
FR_SRC="$REPO_ROOT/content/fr/diary/day-56.mdx"
EN_SRC="$REPO_ROOT/content/en/diary/day-56.mdx"

echo "=== Day 56 Audio Pipeline ==="
echo "Model:  $MODEL"
echo "Voice:  $VOICE_ID"
echo "PROD:   $PROD_URL"
echo "DEV:    $DEV_URL"
...
for locale, src in [('fr', '$FR_SRC'), ('en', '$EN_SRC')]:
    ...
    with open(f'/tmp/day-56-{locale}-input.mdx', 'w', encoding='utf-8') as f:
        f.write(frontmatter + body_new)
PYEOF

python3 "$SCRIPT_DIR/audio-prep.py" /tmp/day-56-fr-input.mdx fr /tmp/day-56-fr.txt
python3 "$SCRIPT_DIR/audio-prep.py" /tmp/day-56-en-input.mdx en /tmp/day-56-en.txt

sed -i 's/PAUSEMARKER/<#1.5#>/g' /tmp/day-56-fr.txt
sed -i 's/PAUSEMARKER/<#1.5#>/g' /tmp/day-56-en.txt

FR_PAUSES=$(grep -o '<#1.5#>' /tmp/day-56-fr.txt | wc -l)
EN_PAUSES=$(grep -o '<#1.5#>' /tmp/day-56-en.txt | wc -l)
FR_CHARS=$(python3 -c "print(len(open('/tmp/day-56-fr.txt', encoding='utf-8').read()))")
EN_CHARS=$(python3 -c "print(len(open('/tmp/day-56-en.txt', encoding='utf-8').read()))")
...
for locale in ['fr', 'en']:
    text = open(f'/tmp/day-56-{locale}.txt', encoding='utf-8').read()
    ...
    with open(f'/tmp/day-56-{locale}-chunk1.txt', 'w', encoding='utf-8') as f:
        f.write(chunk1)
    with open(f'/tmp/day-56-{locale}-chunk2.txt', 'w', encoding='utf-8') as f:
        f.write(chunk2)
PYEOF2
...
FR_C1_RESPONSE=$(curl -s -X POST "https://fal.run/$MODEL" \
  ...
  -d "{\"text\": $(python3 -c "import json; print(json.dumps(open('/tmp/day-56-fr-chunk1.txt', encoding='utf-8').read()))"), \"voice_id\": \"$VOICE_ID\", \"language\": \"fr\"}")
...
FR_C2_RESPONSE=$(curl -s -X POST "https://fal.run/$MODEL" \
  -H "Authorization: Key $FAL_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"text\": $(python3 -c "import json; print(json.dumps(open('/tmp/day-56-fr-chunk2.txt', encoding='utf-8').read()))"), \"voice_id\": \"$VOICE_ID\", \"language\": \"fr\"}")
...
EN_C1_RESPONSE=$(curl -s -X POST "https://fal.run/$MODEL" \
  -H "Authorization: Key $FAL_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"text\": $(python3 -c "import json; print(json.dumps(open('/tmp/day-56-en-chunk1.txt', encoding='utf-8').read()))"), \"voice_id\": \"$VOICE_ID\", \"language\": \"en\"}")
...
EN_C2_RESPONSE=$(curl -s -X POST "https://fal.run/$MODEL" \
  -H "Authorization: Key $FAL_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"text\": $(python3 -c "import json; print(json.dumps(open('/tmp/day-56-en-chunk2.txt', encoding='utf-8').read()))"), \"voice_id\": \"$VOICE_ID\", \"language\": \"en\"}")
...
curl -s -L "$FR_C1_URL" -o /tmp/day-56-fr-c1.mp3
curl -s -L "$FR_C2_URL" -o /tmp/day-56-fr-c2.mp3
curl -s -L "$EN_C1_URL" -o /tmp/day-56-en-c1.mp3
curl -s -L "$EN_C2_URL" -o /tmp/day-56-en-c2.mp3
...
echo "file '/tmp/day-56-fr-c1.mp3'" > /tmp/day-56-fr-list.txt
echo "file '/tmp/day-56-fr-c2.mp3'" >> /tmp/day-56-fr-list.txt
ffmpeg -y -f concat -safe 0 -i /tmp/day-56-fr-list.txt -c copy /tmp/day-56-fr.mp3

echo "file '/tmp/day-56-en-c1.mp3'" > /tmp/day-56-en-list.txt
echo "file '/tmp/day-56-en-c2.mp3'" >> /tmp/day-56-en-list.txt
ffmpeg -y -f concat -safe 0 -i /tmp/day-56-en-list.txt -c copy /tmp/day-56-en.mp3

echo "FR: $(du -sh /tmp/day-56-fr.mp3)"
echo "EN: $(du -sh /tmp/day-56-en.mp3)"
...
entries = [
    ("day-56", "fr", "/tmp/day-56-fr.mp3", "day-56-fr.mp3"),
    ("day-56", "en", "/tmp/day-56-en.mp3", "day-56-en.mp3"),
]
...
echo "  /tmp/day-56-fr.mp3"
echo "  /tmp/day-56-en.mp3"
