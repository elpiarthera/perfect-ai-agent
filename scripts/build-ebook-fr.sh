#!/usr/bin/env bash
# build-ebook-fr.sh
# Builds the FR novel ebook v1 of "L'Agent IA Parfait".
...
# Force UTF-8 locale so pandoc passes through non-ASCII metadata (e.g. ©, é, è) intact.
export LC_ALL="${LC_ALL:-C.UTF-8}"
export LANG="${LANG:-C.UTF-8}"

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="$ROOT/content/fr"
OUT="$ROOT/deliverables/ebook-fr-v1"
COMBINED="$OUT/combined.md"

TITLE="L'Agent IA Parfait"
SUBTITLE="Un Roman"
AUTHOR="Laurent Perello"
PUBLISHER="ElPi Corp"
LANG="fr"
PUBDATE="2026-04-18"
RIGHTS="© 2026 Laurent Perello. Tous droits réservés."

EPUB_OUT="$OUT/PerfectAIAgent-Novel-v1-FR.epub"
PDF_OUT="$OUT/PerfectAIAgent-Novel-v1-FR.pdf"
...
echo "==> Wiping and rebuilding $OUT (preserving cover-fr.png + candidates/ + gumroad-listing-fr.md)"
# Preserve assets that aren't regenerated by the build.
BACKUP_DIR="$(mktemp -d -t ebook-preserve.XXXXXX)"
for ASSET in "cover-fr.png" "gumroad-listing-fr.md"; do
  [[ -f "$OUT/$ASSET" ]] && cp "$OUT/$ASSET" "$BACKUP_DIR/$ASSET"
done
[[ -d "$OUT/candidates" ]] && cp -r "$OUT/candidates" "$BACKUP_DIR/candidates"
rm -rf "$OUT"
mkdir -p "$OUT"
for ASSET in "cover-fr.png" "gumroad-listing-fr.md"; do
  [[ -f "$BACKUP_DIR/$ASSET" ]] && cp "$BACKUP_DIR/$ASSET" "$OUT/$ASSET"
done
[[ -d "$BACKUP_DIR/candidates" ]] && cp -r "$BACKUP_DIR/candidates" "$OUT/candidates"
rm -rf "$BACKUP_DIR"
...
# Strip stray JSX component lines (defensive — current FR novel files have none,
...
# Build EPUB.
COVER_FLAG=()
if [[ -f "$OUT/cover-fr.png" ]]; then
  COVER_FLAG=(--epub-cover-image="$OUT/cover-fr.png")
  echo "==> Using cover: $OUT/cover-fr.png"
fi
