	...
	return {
		...
		alternates: {
			...
			languages: {
				...
				"x-default": `${SITE_URL}/en/what-ai-thinks`,
			},
		},
		openGraph: {
			title: t("meta.title"),
			description: t("meta.description"),
			url: `${SITE_URL}/${locale}/what-ai-thinks`,
	...
	return paragraphs.map((para) => {
		...
		const rendered = parts.map((part) => {
			if (part.startsWith("**") && part.endsWith("**")) {
				return (
					<strong key={`bold-${part}`} className="text-white font-semibold">
						{part.slice(2, -2)}
					</strong>
				);
			}
			...
			if (part.includes("\n- ")) {
				...
				return lines.map((line) => {
					if (line.startsWith("- ")) {
						return (
							<span key={`li-${line}`} className="block pl-4 mt-1">
								&bull; {line.slice(2)}
							</span>
						);
					}
					return <span key={`line-${line}`}>{line}</span>;
				});
			}
			...
			if (part.match(/\n\d+\./)) {
				...
				return lines.map((line) => {
					if (line.match(/^\d+\./)) {
						return (
							<span key={`num-${line}`} className="block pl-4 mt-1">
								{line}
							</span>
						);
					}
		...
		return (
			<p key={`para-${para.slice(0, 40)}`} className="mb-4 leading-relaxed">
				{rendered}
			</p>
		);
	});
		...
		<>
			...
			<div className="max-w-4xl mx-auto px-6">
							...
							<tbody>
								<tr className="border-b border-gray-800">
									...
									<td className="px-4 py-3 text-gray-300">
										{t("comparison.claudeEmotional")}
									</td>
									<td className="px-4 py-3 text-gray-300">
										{t("comparison.claudeSins")}
									</td>
									<td className="px-4 py-3 text-gray-300">
										{t("comparison.claudeForgetting")}
									</td>
								</tr>
								<tr className="border-b border-gray-800 bg-surface/50">
									...
									<td className="px-4 py-3 text-gray-300">
										{t("comparison.geminiEmotional")}
									</td>
									<td className="px-4 py-3 text-gray-300">
										{t("comparison.geminiSins")}
									</td>
									<td className="px-4 py-3 text-gray-300">
										{t("comparison.geminiForgetting")}
									</td>
								</tr>
								<tr className="border-b border-gray-800">
									<td className="px-4 py-3 text-white font-semibold whitespace-nowrap">
									...
									<td className="px-4 py-3 text-gray-300">
										{t("comparison.chatgptEmotional")}
									</td>
									<td className="px-4 py-3 text-gray-300">
										{t("comparison.chatgptSins")}
									</td>
									<td className="px-4 py-3 text-gray-300">
										{t("comparison.chatgptForgetting")}
									</td>
								</tr>
								<tr>
									<td className="px-4 py-3 text-white font-semibold whitespace-nowrap">
									...
									<td className="px-4 py-3 text-gray-300">
										{t("comparison.grokEmotional")}
									</td>
									<td className="px-4 py-3 text-gray-300">
										{t("comparison.grokSins")}
									</td>
									<td className="px-4 py-3 text-gray-300">
										{t("comparison.grokForgetting")}
									</td>
								</tr>
							</tbody>
						</table>
				...
				<section className="py-16 border-t border-gray-800 text-center max-w-2xl mx-auto">
					...
					<p className="text-muted mt-4 text-sm font-sans">
